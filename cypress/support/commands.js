...
// e.g. catagotyName = Tshirt => tshrit, URL = http://automationpractice.com/index.php?id_category=5&controller=category
...
Cypress.Commands.add('findAndSelectProductFromCard', (order) => {
    ...
    .find(`.product-name[title="${order['name']}"][href="http://automationpractice.com/index.php?id_product=${order['id']}&controller=product"]`)
    .closest('li')
...
Cypress.Commands.add('selectedProductFromProductDetail', (order) => {
    ...
    cy.get(`.primary_block`)
        ...
        .then($form => {
            ...
            cy.editProductQtyFromInputOrIcon(order['editThisOrderQtyFrom'], '#quantity_wanted_p');
...
Cypress.Commands.add('editProductQtyFromInputOrIcon', (editThisOrderQtyFrom, $mainElement = '') => {
            ...
            .then($ele => {
                ...
                if(nowQty !== editQty) {
                    ...
                    if(editThisOrder.includes('Input')) {
                        ...
                        cy.get($ele).find(inputElement).clear().type(editQty, { force: true });
    ...
    if(options['type'] === 'checkOrdersDetail') {
        ...
        cy.get($tableId)
            ...
            .then($td => {
                ...
                // Quantity: Edit product's qty by.Input or Click icon
                cy.editProductQtyFromInputOrIcon(order['editThisOrderQtyFrom'], $td);
        });
